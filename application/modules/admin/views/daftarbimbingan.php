            ...
            <div class="box-body">
                ...
                <form id="form" method="post" action="<?=base_url('admin') . "/" . $this->uri->segment(2)?>/export">
                    <div class="pull-Left col-sm-12" style="margin-bottom: 5px;">
                        <div class="col-sm-3">
                            <select class="pull-right form-control input-sm" id="st_semester" name="st_semester" title="Pilih Semester">
                                <option value="" disabled>-- Pilih Semester --</option>
                                <?php foreach ($semester_lists as $key => $value) { ?>
                                    
                                    <?php $current_year = (int)substr($value->semester, 0, 4); ?>
                                    <?php $next_year = $current_year + 1; ?>
                                    <?php $smt = substr($value->semester, 4, strlen($value->semester)) ?>
                                    <?php $smt_val = ($smt == '1') ? 'Gasal' : 'Genap'; ?>

                                    <option value="<?=$value->semester?>" <?php if ($value->semester == $current_semester): echo "selected"; endif; ?>>Semester <?=$current_year?>/<?=$next_year?> <?=$smt_val?></option>
                                <?php } ?>
                            </select>
                        </div>
                ...
                <table id="table-detail" cellpadding="0" cellspacing="0" border="1" class="table table-bordered table-striped" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 7%">Jumlah Mahasiswa</th>
